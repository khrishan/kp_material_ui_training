...
const Estimate = () => {
    ...
    const estimateDisabled = () => {
        ...
        const emptySelections = questions.filter(question => question.title !== 'Which features do you expect to use?').map(question => question.options.filter(option => option.selected)).filter(question => question.length === 0)

        const featureSelections = questions.filter(question => question.title === 'Which features do you expect to use?').map(question => question.options.filter(option => option.selected)).filter(selections => selections.length > 0 )
        ...
        if (questions.length === 2 ){
            ...
            disabled =  true
        } else if (emptySelections.length === 1 && features.selected.length > 0) {
            disabled =  false
        }

        

        console.log(featureSelections)
    ...
    return (
        <Grid container direction='row'>
            <Head>
                ...
                <meta property='og:url' key='og:url' content='https://arc.com/estimate' />
            ...
            <Grid item container direction='column' lg alignItems={matchesMD ? 'center' : undefined}>
                ...
                <Grid item container direction='column' style={{ marginRight: matchesMD ? 0 : '10em', maxWidth:'50em', marginTop: '7.5em'}}>
            ...
            <Grid item container direction='column'lg style={{marginRight: matchesMD ? 0 : '2em', marginBottom: '25em'}} alignItems='center'>
                {questions.filter(question => question.active).map((question, index) => (
                    <React.Fragment key={index}>
                        <Grid item ref={myRef}>
                            <Typography variant='h1' align='center' style={{fontWeight: 500, fontSize: '2.25em', marginTop : '5em', lineHeight: '1.25em', marginLeft : matchesSM ? '1em' : 0, marginRight : matchesSM ? '1em' : 0}}>{question.title}</Typography>
                ...
                <Grid item>
                    <Button variant='contained' className={classes.estimateButton} disabled={estimateDisabled()} onClick={() => {setDialogOpen(true); getTotal(); getPlatforms(); getFeatures(); getCustomFeatures(); getCategory(); ReactGA.event({category: 'Estimate', action: 'Estimate Checked'})}}>Get Estimate</Button>
                </Grid>
            </Grid>
                        ...
                        <Grid item container direction='column' md={5} style={{maxWidth: '30em'}} alignItems={matchesSM ? 'center' : undefined}>
                            ...
                            <Grid item>
                                <Button disabled={name.length === 0 || message.length === 0 || phoneHelper.length !== 0 || emailHelper.length !== 0 } variant='contained' variant='contained' className={classes.estimateButton} onClick={sendEstimate}>
                                    {loading ? <CircularProgress /> : <React.Fragment>Place Request <img src='/assets/send.svg' alt='paper airplane' style={{marginLeft: '0.5em'}}/></React.Fragment>}
