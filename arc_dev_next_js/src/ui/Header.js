...
const Header = (props) => {
    ...
    const tabs = (
        <React.Fragment>
            ...
            <Tabs value={props.value} onChange={handleChange} className={classes.tabContainer} scrollButtons='auto'>
            ...
            <Button component={Link} href='/estimate' variant='contained' color='secondary' className={classes.button} size='large' onClick={() => {props.setValue(5); ReactGA.event({category: 'Estimate', action: 'Desktop Header Pressed'})}}>
                Free Estimate
            </Button>
            <Popper open={openMenu} anchorEl={anchorEl} role={undefined} transition disablePortal placement='bottom-start'>
                {({ TransitionProps, placement }) => (
                    ...
                    <Paper classes={{root: classes.menu}} elevation={0}>
                        ...
                        <MenuList onMouseOver={() => setOpenMenu(true)} autoFocusItem={false} id="simple-menu" onMouseLeave={handleClose}  disablePadding onKeyDown={handleListKeyDown}>
                            {
                                menuOptions.map((option, i) => (
                                    <MenuItem key={`${option}${i}`} onClick={(event) => {handleMenuItemClick(event, i); props.setValue(1); handleClose()}} selected={i === props.selectedIndex && props.value === 1 && window.location.pathname !== '/services' } component={Link} href={option.link} classes={{root: classes.menuItem}}>{option.name}</MenuItem>
                                ))
                            }
                        </MenuList>
                        </ClickAwayListener>
            ...
            <SwipeableDrawer classes={{paper: classes.drawer}}anchor='right' disableBackdropTransition={!iOS} disableDiscovery={iOS} open={openDrawer} onClose={() => setOpenDrawer(false)} onOpen={() => setOpenDrawer(true)}>
                ...
                <List disablePadding>
                    {routes.map(route => route.name === 'Services' ? (
                        <ExpansionPanel classes={{root: classes.expansion}} key={route.name} elevation={0}>
                            <ExpansionPanelSummary expandIcon={<ExpandMoreIcon color='secondary' />} classes={{ root : classes.expansionSummary}}>
                                <Link color='inherit' href={route.link}><ListItemText opacity={{ opacity : props.value === 1 ? 1 : null }} onClick={() => {setOpenDrawer(false); props.setValue(route.activeIndex)}} className={classes.drawerItem} disableTypography>{route.name}</ListItemText></Link>
                            </ExpansionPanelSummary>
                            <ExpansionPanelDetails classes={{root : classes.expansionDetails}}>
                                <Grid container direction='column'>
                                    {menuOptions.map(route => (
                                        <Grid item>
                                            <ListItem key={`${route}${route.selectedIndex}`} onClick={() => {setOpenDrawer(false); props.setValue(route.selectedIndex)}} component={Link} href={route.link} divider button selected={props.selectedIndex === route.selectedIndex && props.value === 1 && window.location.pathname !== '/services'} classes={{selected: classes.drawerItemSelected}}>
                        ...
                        <ListItem key={`${route}${route.selectedIndex}`} onClick={() => {setOpenDrawer(false); props.setSelectedIndex(route.selectedIndex)}} component={Link} href={route.link} divider button selected={props.value === route.activeIndex} classes={{selected: classes.drawerItemSelected}}>
                            <ListItemText className={classes.drawerItem} disableTypography>{route.name}</ListItemText>
                        </ListItem>
                    ))}
                    <ListItem className={{root: classes.drawerItemEstimate,  selected: classes.drawerItemSelected }} onClick={() => {setOpenDrawer(false); props.setValue(5); ReactGA.event({category: 'Estimate', action: 'Mobile Header Pressed'})}} component={Link} href='/estimate' divider button  selected={props.value === 5}>
        ...
        <React.Fragment>
            <AppBar color='primary' className={classes.appbar}>
                <Toolbar disableGutters>
                    <Button onClick={() => props.setValue(0)} component={Link} href='/' className={classes.logoContainer} disableRipple style={{textDecoration : 'none'}}>
